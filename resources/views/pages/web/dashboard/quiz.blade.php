...
<main class="flex-grow">
    ...
    <!-- Hidden input untuk data peserta -->
...
<script>
    ...
    function renderQuestion(index) {
        ...
        // Tampilkan/hilangkan tombol back
        backBtn.classList.toggle('hidden', currentQuestion === 0);

        // Ganti label tombol Next jadi Finish jika di soal terakhir
        nextBtn.textContent = currentQuestion === questions.length - 1 ? 'Finish' : 'Next';

        // Scroll ke atas saat soal berpindah
    ...
    function renderPagination() {
        ...
        if (current < total - range - 3) {
            pagination.appendChild(createEllipsis());
        }

        if (total > 1) {
            pagination.appendChild(createButton(total - 1));
        }
    }
    ...
    function kirimTest() {
        ...
        if (unanswered > 0) {
            if (!confirm(`Masih ada ${unanswered} soal belum dijawab. Kirim tetap?`)) return;
        }
